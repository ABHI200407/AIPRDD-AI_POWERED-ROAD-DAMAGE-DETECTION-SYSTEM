...
import { auth, googleProvider, signInWithPopup, RecaptchaVerifier, signInWithPhoneNumber, signInWithEmailAndPassword, createUserWithEmailAndPassword } from './firebase';
import { Shield, Smartphone, Globe, LogIn, ChevronRight, X, Mail } from 'lucide-react';

export default function LoginView({ onLogin, onGuest }) {
  const [method, setMethod] = useState('select'); // 'select', 'phone', 'email'
  ...
  return (
    <div style={{ height: '100%', width: '100%', background: 'linear-gradient(to bottom, #0f172a, #020617)', display: 'flex', flexDirection: 'column', color: 'white', position: 'absolute', top: 0, left: 0, zIndex: 9999 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px' }}>
        ...
        <h1 style={{ fontSize: '2rem', fontWeight: 900, marginBottom: 8, letterSpacing: '-0.02em', textAlign: 'center' }}>AIPRDD: Road Sentinel</h1>
        <p style={{ color: '#94a3b8', textAlign: 'center', marginBottom: 40, lineHeight: 1.5, fontSize: '0.95rem' }}>
          AI-Powered Road Damage Detection & SafePath Navigation. Help make city roads safer.
        </p>
        ...
        <div style={{ width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {method === 'email' && (
            <form onSubmit={handleEmailAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: 8, fontSize: '0.85rem', color: '#cbd5e1', fontWeight: 600 }}>Email Address</label>
                <input 
                  type="email" 
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="citizen@city.com" 
                  style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white', fontSize: '1rem', outline: 'none' }}
                  required
                />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: 8, fontSize: '0.85rem', color: '#cbd5e1', fontWeight: 600 }}>Password</label>
                <input 
                  type="password" 
                  value={password}
              ...
              <button 
                type="submit" 
                disabled={loading}
                style={{ width: '100%', padding: '16px', borderRadius: '12px', border: 'none', background: '#f43f5e', color: 'white', fontWeight: 800, fontSize: '1rem', cursor: 'pointer' }}
              >
              ...
              <button 
                ...
                style={{ background: 'transparent', border: 'none', color: '#94a3b8', fontSize: '0.85rem', cursor: 'pointer', marginTop: 8 }}
              >
