...
export default function SentinelView({ onClose, onDetect }) {
    ...
    canvasRef.current.toBlob(async (blob) => {
      ...
      try {
        ...
        if (data.status === 'success' && data.detections?.length > 0) {
          ...
          reportToGov(primary, data.image_url);
  ...
  const reportToGov = async (detection, imageUrl) => {
    const payload = {
      ...
      assessment: { 
        damage_type: detection.class_name || detection.type, 
        ai_suggested_severity: detection.severity_score || detection.severity 
      },
      source: 'SENTINEL',
