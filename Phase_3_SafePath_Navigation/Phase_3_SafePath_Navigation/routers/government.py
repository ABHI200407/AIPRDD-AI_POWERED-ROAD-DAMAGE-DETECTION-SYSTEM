...
def aggregate_reports_to_tickets(db: Session = Depends(get_db)):
    ...
    Requires either 3 distinct reports, 3 total verification signals, or accelerometer impact verification before queue impact.
    """
    # Only process non-flagged reports that are NOT already part of a MasterTicket
    reports = db.query(RoadDamageReport).filter(
        RoadDamageReport.is_flagged == False,
        RoadDamageReport.master_ticket_id == None
    ).all()
    ...
    for label, cluster_reports in clusters.items():
        ...
        total_signals = len(cluster_reports) + sum((r.verification_count or 0) for r in cluster_reports)
        has_accel_verification = any(r.is_impact_verified for r in cluster_reports)
        
        # If no existing ticket, must meet threshold (>=3 signals OR accelerometer verified)
        if not existing_ticket and total_signals < 3 and not has_accel_verification:
            continue
            
        reports_processed += len(cluster_reports)
        ...
        if existing_ticket:
            ...
            for r in cluster_reports:
                r.master_ticket_id = existing_ticket.ticket_id
        else:
            max_sev = max([r.user_confirmed_severity or r.ai_suggested_severity or 1 for r in cluster_reports])
            ...
            for r in cluster_reports:
                r.master_ticket_id = new_ticket_id
                
            tickets_created += 1
            
    db.commit()
    return {"status": "success", "tickets_created": tickets_created, "reports_processed": reports_processed}
