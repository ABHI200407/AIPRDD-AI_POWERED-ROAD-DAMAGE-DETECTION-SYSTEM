...
async def analyze_image_remote(file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 1. Upload to Cloudinary first to get a permanent URL
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                result["image_url"] = image_url  # Attach the URL for the frontend
                return result
            else:
                return {
                    "status": "error", 
                    "message": f"HF Space returned {response.status_code}", 
                    "detections": [], 
                    "top_class": "unknown",
                    "image_url": image_url
                }
...
async def create_report(report: ReportCreate, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    db_report = RoadDamageReport(
        ...
        raw_image_url=report.media.raw_image_url if report.media else None,
        ml_cropped_image_url=report.media.ml_cropped_image_url if report.media else None
    )
